...
class Joint:
    def __init__(self,position,parent_name):
        ...
        self.actuator.enableTorqueFeedback(8)
...
def define_start_stop_coordsV3(want_stop,distance,angle,leg_index):
    ...
    if int(converter.convert_angle(angles_start[0]*(math.pi/180),leg_index)*(180/math.pi)) not in range(*sorted((int(_min),int(_max)))) or  int(converter.convert_angle(angles_end[0]*(math.pi/180),leg_index)*(180/math.pi)) not in range(*sorted((int(_min),int(_max)))) or angles_end[0] == 404 or angles_start[0] == 404:
        # print("mark")
        ...
        while int(converter.convert_angle(angles_start[0]*(math.pi/180),leg_index)*(180/math.pi)) not in range(*sorted((int(_min),int(_max)))) or int(converter.convert_angle(angles_end[0]*(math.pi/180),leg_index)*(180/math.pi)) not in range(*sorted((int(_min),int(_max)))) or angles_end[0] == 404 or angles_start[0] == 404:
            a = converter.translate_coord(start_coord,leg_index,1)
            angles_start  = control.IK(a)
            a = converter.translate_coord(end_coord,leg_index,1)
            angles_end  = control.IK(a)
            if idx > 1000:
                break
            #print("angles")
            #print(int(converter.convert_angle(angles_start[0]*(math.pi/180),leg_index)*(180/math.pi)))
            #print(int(converter.convert_angle(angles_end[0]*(math.pi/180),leg_index)*(180/math.pi)))
    ...
    for i in range(8):
        ...
        if i == 0:
            ...
            # if time - time_log > 3:
                ...
                # print("mark")
                # spider_angle = 2*math.pi*random.random()-math.pi
                # if spider_speed < 0:
                    # angle = spider_angle
                # else:
                    # if spider_angle == 0:
