...
#representation
representation = np.array([0.6,      50,        300,       1,         -250,       200,       1,         180,        0,         45,        90,        135,       180,       225,       270,       315,       200,       100,       100,       200,       100,       200,       200,       100,       200,       100,       100,       200,       100,       200,       200,       100,       80])
val_type =       np.array([['f','u'],['f','if'],['i','if'],['f','if'],['i','if'], ['i','if'],['f','if'],['i','if'],['i','if'],['i','if'],['i','if'],['i','if'],['i','if'],['i','if'],['i','if'],['i','if'],['i','if'],['i','if'],['i','if'],['i','if'],['i','if'],['i','if'],['i','if'],['i','if'],['i','if'],['i','if'],['i','if'],['i','if'],['i','if'],['i','if'],['i','if'],['i','if'],['i','if']])
min_max =        np.array([[0,2],    [0,1000],  [0,359],   [0,180],   [-250,0],   [0,1000],  [0,10],    [0,300],   [0,359],   [0,359],   [0,359],   [0,359],   [0,359],   [0,359],   [0,359],   [0,359],   [1000,0],  [1000,0],  [1000,0],  [1000,0],  [1000,0],  [1000,0],  [1000,0],  [1000,0],  [1000,0],  [1000,0],  [1000,0],  [1000,0],  [1000,0],  [1000,0],  [1000,0],  [1000,0],  [1000,0]])

if np.size(representation,axis = 0) != np.size(val_type,axis = 0) != np.size(min_max,axis = 0):
    print(np.size(representation,axis = 0))
    print(np.size(val_type,axis = 0))
    print(np.size(min_max,axis = 0))
    raise ValueError("unequal sizes")

#Hyperpartmaetrs
##max iteration
max_fit = 10000
elites_percentage = 0.4
popsize = 10
crossover_prob = 0.9
mut_mu = 6
fitness_time = 5

max_gen = math.floor(max_fit/popsize)
num_elites = math.ceil(popsize*elites_percentage)
num_parent = popsize-num_elites
mutation_prob = 1/np.size(representation)*2
...
#intial distribution
init_pop = np.zeros((popsize,np.size(representation)))

for i in range(popsize):
    for j in range(np.size(representation)):
        if val_type[j,1] == 'u':
            rand = random.random()
            val = abs(min_max[j,0]-min_max[j,1])*rand + min(min_max[j,0],min_max[j,1])
        
        if val_type[j,1] == 'f' or val_type[j,1] == 'if':
            val = representation[j]
        
        if val_type[j,1] == 'n': 
            
            rand =  truncnorm.rvs(0, 1, size=1)
            print(rand)
            if rand < 0.5:
                val = rand*2*(representation[j]-min_max[j,0])+min_max[j,0]
            else:
                val = 2*(rand-0.5)*(min_max[j,1]-representation[j])+representation[j]
            #val = abs(min_max[j,0]-min_max[j,1])*rand + min(min_max[j,0],min_max[j,1])
...
baseline_fit = fitness_solo(representation)
best_fitness_arr=[baseline_fit]

print("baseline fitness: "+str(baseline_fit))

while robot.step(timestep) != -1:
    for i in range(max_gen):
        print(pop)
        print("current gteneration: "+str(i))
        print("Progress: "+str(i/max_gen*100)+"%")
        
    ##fitness caluclation
        fitness  = np.zeros((popsize,2))
        for j,val in enumerate(pop[:,:]):
            fitness[j,0] = j
            print("Fitnesse test "+str(j+1)+" of "+str(popsize))
            fitness[j,1] = fitness_solo(val)
            print(fitness[j,1]-baseline_fit)
        # print(fitness)
        fitness = fitness[fitness[:, 1].argsort()[::-1]]
        # print(fitness)
        #print(fitness[0,1])
        print(fitness)
        print(pop[int(fitness[0,0]),:])
               
    ##elites
        elites = np.zeros((popsize,np.size(representation)))
        for j in range(num_elites):
            elites[j,:] = pop[int(fitness[j,0]),:]
            
        # print(elites)
    ##selection
        FCD = np.zeros((popsize,1))
        for j in range(popsize):
            if j == 0:
                FCD[j] = fitness[j,1]
            else:
                FCD[j] = FCD[j-1]+fitness[j,1]

        children = np.zeros((num_parent,np.size(representation)))
        for j in range(num_parent):
            rand1 = random.random()*FCD[popsize-1]
            rand2 = random.random()*FCD[popsize-1]
            pos1 = 0
            for n,m in enumerate(FCD):
                if m < rand1:
                    pos1 = n+1
            pos2 = 0
            for n,m in enumerate(FCD):
    ...
    ##reproduction
            ...
            child = np.zeros((np.size(representation)))
            for n in range(np.size(representation)):
                rand = random.random()
                if rand < 0.5:
                    child[n] = parent1[n]
                else:
    ...
    ##mutation        
            for n in range(np.size(representation)):
                ...
                if rand < mutation_prob:
                    ...
                    if val_type[n,1] == 'f':
                        child[n] = representation[n]
                        
                
                    elif val_type[n,0] == 'i':
                        rand = random.random()
                        val = abs(min_max[n,0]-min_max[n,1])*rand + min(min_max[n,0],min_max[n,1])
                        child[n] = round(float(val),0) 
                        # coin = random.random()
                        # if coin <= 0.5 and child[n]+1 <= max(min_max[n,0],min_max[n,1]):
                            # child[n] = child[n]+1
                        # elif coin > 0.5 and child[n]-1 >= min(min_max[n,0],min_max[n,1]):
                            # child[n] = child[n]-1 
                        # elif coin <= 0.5 and child[n]+1 > max(min_max[n,0],min_max[n,1]):
                            # child[n] = max(min_max[n,0],min_max[n,1])
                        # elif coin > 0.5 and child[n]-1 < min(min_max[n,0],min_max[n,1]):
                            # child[n] = min(min_max[n,0],min_max[n,1])
                            
                           
                        
                    elif val_type[n,0] == 'f':
                        rand = random.random()
                        child[n] = abs(min_max[n,0]-min_max[n,1])*rand + min(min_max[n,0],min_max[n,1])
                        #mut_rand = truncnorm.rvs(0, 1, size=1)
                        #if child[n]+min(mut_mu/100*abs(min_max[n,0]-min_max[n,1])*mut_rand,mut_mu*mut_rand)-min((mut_mu/2)/100*abs(min_max[n,0]-min_max[n,1])*mut_rand,mut_mu/2*mut_rand) > max(min_max[n,0],min_max[n,1]):
                        #    child[n] = child[n] -min((mut_mu/2)/100*abs(min_max[n,0]-min_max[n,1])*mut_rand,mut_mu/2*mut_rand)
                        #elif child[n]+min(mut_mu/100*abs(min_max[n,0]-min_max[n,1])*mut_rand,mut_mu*mut_rand)-min((mut_mu/2)/100*abs(min_max[n,0]-min_max[n,1])*mut_rand,mut_mu/2*mut_rand) < min(min_max[n,0],min_max[n,1]):
                        #    child[n] = child[n] +min((mut_mu/2)/100*abs(min_max[n,0]-min_max[n,1])*mut_rand,mut_mu/2*mut_rand)
                        #else:
                        #    child[n] = child[n]+min(mut_mu/100*abs(min_max[n,0]-min_max[n,1])*mut_rand,mut_mu*mut_rand)-min((mut_mu/2)/100*abs(min_max[n,0]-min_max[n,1])*mut_rand,mut_mu/2*mut_rand)
            children[j,:] = child

    ##new population
        pop = elites
        
        for j in range(popsize-num_elites):
            pop[j+num_elites,:] = children[j,:]
            
        np.savetxt("population_log.txt", pop)
